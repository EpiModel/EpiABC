...
#' 
#' @param method a character string indicating the sequential algorithm to be
#' used. Possible values are \code{"Beaumont"}, \code{"Drovandi"},
#' \code{"Delmoral"}, \code{"Lenormand"} and \code{"Emulation"}.
...
#' distribution.  Note that when using the method "Lenormand", solely uniform
#' prior distributions are supported. User-defined prior distributions can also
#' be provided. See the vignette for additional information on this topic.
#' @param nb_simul a positive integer equal to the desired number of
#' simulations of the model below the tolerance threshold when \code{method} is
#' \code{"Beaumont"}, \code{"Drovandi"} and \code{"Delmoral"}.  When
#' \code{method} is \code{"Lenormand"}, the number of simulations below the
#' tolerance threshold is equal to \code{nb_simul * alpha}. See the package's
#' vignette and Lenormand et al. (2012) for details.
...
#' parameters used for the simulation.  If \code{TRUE}, \code{ABC_sequential}
...
#' \code{ABC_sequential} writes in the current directory intermediary results
#' at the end of each step of the algorithm various files.  The file
...
#' "p_acc_iteration" (when using the method "Lenormand") is the parameter p_acc
#' computed at the end of the step "iteration" (see Lenormand et al. 2012 for
#' details).  The file "tolerance_step_iteration" (when using the method
#' "Drovandi", "Delmoral" or "Lenormand") is the tolerance computed at the end
#' of the step "iteration".  The file "output_step_iteration" gives the
#' simulations kept after each iteration and has a matrix format, in wich each
#' row is a different simulation, the first column is the weight of the
#' simulation, the following columns are the parameters used for this
#' simulation, and the last columns are the summary statistics of this
#' simulation.  The file "model_step_iteration" gives the simulations performed
#' at each iteration and has a matrix format, in which each row is a different
#' simulation, the first column is the weight of the simulation, the following
#' columns are the parameters used for this simulation, and the last columns
#' are the summary statistics of this simulation.  All these informations are
#' further stored in a list (with the same formats) and are accessible from R -
#' see \code{intermediary} in the value section below.
...
#' simulations.} \item{stats_normalization}{ The standard deviation of the
#' summary statistics across the \code{model} simulations of the initial step.
#' These values are used to normalize the summary statistics before the
#' computation of the Euclidean distance between simulations and data.}
#' \item{epsilon}{ The final maximal distance between simulations and data in
#' the retained sample of particles.} \item{nsim}{ The number of \code{model}
#' simulations performed.} \item{computime}{ The computing time to perform the
#' simulations.} \item{intermediary}{ Intermediary results stored when the
#' option \code{"verbose=TRUE"} is chosen. Each element of this list
#' corresponds to a different step. See the argument \code{verbose} above for
#' more details on the information stored.}
#' @section Additional parameters: Depending on the choosen method, you can
#' specify the following arguments: \describe{ \item{seed_count}{ a positive
...
#' \code{\link{ABC_mcmc}}
#' @references Beaumont, M. A., Cornuet, J., Marin, J., and Robert, C. P.
#' (2009) Adaptive approximate Bayesian computation.
#' \emph{Biometrika},\bold{96}, 983--990.
#' 
#' Del Moral, P., Doucet, A., and Jasra, A. (2012) An adaptive sequential Monte
#' Carlo method for approximate Bayesian computation. \emph{Statistics and
#' Computing}, \bold{22}, 1009--1020.
#' 
...
#' \emph{Biometrics}, \bold{67}, 225--233.
#' 
#' Lenormand, M., Jabot, F., Deffuant G. (2012) Adaptive approximate Bayesian
#' computation for complex models. http://arxiv.org/pdf/1111.1308.pdf
#' 
#' Jabot, F., Lagarrigues G., Courbaud B., Dumoulin N. (2015). A comparison of
#' emulation methods for Approximate Bayesian Computation. To be published.
#' @keywords abc model inference sequential_monte_carlo population_monte_carlo
#' @examples
#' 
#'  \dontrun{ 
#'     ##### EXAMPLE 1 #####
...
#' @export ABC_sequential
ABC_sequential_MPI <- function(method, model, prior, nb_simul, summary_stat_target, 
                               prior_test = NULL, n_cluster = 1, use_seed = FALSE, 
                               verbose = FALSE, dist_weights=NULL, cl, ...) {
    ## checking errors in the inputs
    if (missing(method)) 
        stop("'method' is missing")
    if (missing(model)) 
